...
function createRenderer(options) {
  const { createElement, setTextContent, insert, patchProps, setText, createText } = options;
  ...
  function patch(n1, n2, container) {
    ...
    if (typeof type === "string") {
      ...
      if (!n1) {
        const element = (n2.element = document.createTextNode(n2.children));
        insert(element, container);
      } else {
        const element = (n2.element = n1.element);
        if (n2.children !== n1.children) {
          // 调用 setText 函数更新文本节点的内容
          setText(element, n2.children);
        }
      }
